...
def create_rag(source, questions, sheet_breakdown_fields, suppression_threshold, output_path):
    RagReport(source, output_path, questions, sheet_breakdown_fields, suppression_threshold)
...
class RAGWorkbook(report.ReportWorkbook):
    ...
    def create_guidance_page(self):
        ...
        guidance_sheet.merge_range(7, 1, 7, 13, 'This report shows positive scores for the reakdown categories_for_output compared with the combined score. It is a dynamic report you can adjust to get the maximum insight from your data.', self.formats['RAG_GUIDANNCE'])
        guidance_sheet.merge_range(8, 1, 8, 13, 'By default the RAG comparison is set at 3 or more percentage points difference between the scores. To highlight all differences of 5 percentage points or more adjust cell E6 to 5, to see all differences of 10 percentage points or more change this to 10 etc. Red differences are where the scores are more than this value below the organisation average, green differences are more than this value above the organisation average.', self.formats['RAG_GUIDANNCE'])
        guidance_sheet.merge_range(9, 1, 9, 13, 'The positive score is the percentage of respondents to whom the question applies,  who who gave a favourable response to each question. Values are rounded to the nearest percent. Only questions that can be positively scored have been included.', self.formats['RAG_GUIDANNCE'])
        guidance_sheet.merge_range(10, 1, 10, 13, 'If there is a suppression threshold for your survey, where there are less than x responses to a question (or zero),  the respective scores are replaced with an asterisk (*)', self.formats['RAG_GUIDANNCE'])
...
class RAGWorksheet(report.ReportWorksheet):
    ...
    def __init__(self, sheet_data, worksheet, sheet_breakdown, parent_workbook):
        ...
        self.worksheet.insert_image('A1', frequency_table_config.LOGO_PATH, {'y_offset': 15, 'x_offset': 10, 'x_scale': 0.85, 'y_scale': 0.85})
        ...
        self.worksheet.write(2, 2, 'RAG Tables Report', self.formats['SUBTITLE'])
        self.worksheet.write(3, 2, f'Broken Down By: {self.breakdown_text}', self.formats['SUBTITLE'])
        self.worksheet.write(1, 3, 'RAG percentage', self.formats['SUBTITLE'])
        self.worksheet.write(2, 3, 'point difference:', self.formats['SUBTITLE'])
        ...
        if self.number_of_breakdowns > 1:
            self.worksheet.merge_range(6, 3, self.number_of_breakdowns+5, 3, 'Overall', self.formats['HEADER'])
        else:
            self.worksheet.write(self.number_of_breakdowns+5, 3, 'Overall', self.formats['HEADER'])
        ...
        self.worksheet.conditional_format(self.number_of_breakdowns+6, 4, self.number_of_breakdowns+number_questions+6, number_columns+3, {'type': 'formula',
